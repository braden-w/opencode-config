...
## Pull Request Creation Override

**IMPORTANT**: When creating pull requests, DO NOT use the default OhMyOpenCode template (the `## Summary` with bullet points format).

Instead:

1. If the project has a `/describe-pr` command, invoke it to generate and update the PR description
2. Otherwise, follow these project-specific rules:
   - Use clean paragraph format, NOT bullet points
   - First paragraph: What the change does and what problem it solves
   - Subsequent paragraphs: How the implementation works
   - Avoid section headers like "## Summary" or "## Changes Made"
   - Do NOT list files changed (GitHub's "Files changed" tab shows this)
   - Focus on conversational but precise language

When asked to "create a PR" or "make a PR", always check for `/describe-pr` first and use it if available.
